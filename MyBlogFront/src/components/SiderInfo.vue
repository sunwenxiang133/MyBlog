<template>
  <div id="siderInfo-box">
    ...
    <div class="card" :style="`${color};${mainBg}`">
      <div class="card-head">
        <i class="el-icon-price-tag" style="margin-right: 10px"></i>书签
      </div>
      <span
    ...
    <div class="card" :style="`${color};${mainBg}`">
      <div class="card-head">
        <i class="el-icon-medal" style="margin-right: 10px"></i>最新心情
      </div>
      <div
        class="card-item"
      ...
      <el-empty
        v-if="jottingList.length == 0"
        description="博主正在加班创作中！"
      ></el-empty>
    </div>
    <div class="card" :style="`${color};${mainBg}`">
      <div class="card-head">
  ...
  methods: {
    ...
    async getBaseInfo() {
      try {
        if (
          this.blogs.length > 0 &&
          this.jottings.length > 0 &&
          this.classifies.length > 0
        ) {
          ...
          if (res.status === 200) {
            ...
            if (res.data.jottings.length > 0)
              this.jottingList = res.data.jottings.slice(0, 3);
